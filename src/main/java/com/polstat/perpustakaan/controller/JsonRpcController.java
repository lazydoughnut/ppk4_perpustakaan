        ...
        try {
            ...
            switch (method) {
                ...
                case "searchBooks":
                    ...
                    String keyword = "";
                    if (params != null && params.has("keyword") && !params.get("keyword").isNull()) {
                        keyword = params.get("keyword").asText();
                    }

                    List<BookDto> matched = bookService.searchBooks(keyword);
                    return ResponseEntity.ok(new JsonRpcResponse(matched, request.getId()));
